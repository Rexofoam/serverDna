<?php
    ...
    if (duplicateMembersExist($team_capt, $team_vice, $team_member_arr, $team_sub_arr)) {
        ...
        $return_val = mysqli_query($con,"SELECT COUNT(*) AS 'Number' FROM `teams` WHERE UPPER('$team_name') = UPPER(team_name) AND `team_id` != '$team_id' AND status IN ('pending', 'authenticated')");
